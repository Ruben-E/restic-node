...
const additionalOptions = process.env.RESTIC_ADDITIONAL_OPTIONS;
const additionalOptionsString = additionalOptions
  ? `-o ${additionalOptions}`
  : "";

// const rcloneArgs = `serve restic --stdio --b2-hard-delete --drive-use-trash=false --fast-list --transfers=32`;

const backupCommand = `restic backup /data ${backupArgs} ${additionalOptionsString}`;
const forgetCommand = `restic forget ${forgetArgs} ${additionalOptionsString}`;
const checkWeeklySubsetCommand = `restic check ${checkArgs} --read-data-subset=<week>/52 ${additionalOptionsString}`;
const pruneCommand = `restic prune ${pruneArgs} ${additionalOptionsString}`;
